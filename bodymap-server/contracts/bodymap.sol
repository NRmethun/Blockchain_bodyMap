...
contract BodyMap {
    ...
    function setBodyMaps(
        string memory passwordClearText,
        string memory newBasicBodyMap,
        string memory newTailorBodyMap
    ) public {
        bytes32 givenPasswordHash = keccak256(
            abi.encodePacked(passwordClearText)
        );
