...
function App() {
  ...
  const [deployedAddres, setDeployedAddress] = useState('');
  ...
  async function updateBodyMaps() {
    ...
    await deployedContract.methods.setBodyMaps(passwordClearText, encryptedBodyMapBasic, encryptedBodyMapTailor).send({ from: accounts[0], gas: 100000 });
  }
  ...
  return (
    <div className="App">
      ...
      <h3>Contract address: {deployedAddres} </h3>
      <div>
      ...
      {Object.keys(bodyMapBasic).map((key) => (
        <div key={key}>
          <label> {key}: </label> <input type="text" value={bodyMapBasic[key]} onChange={(e) => handleBodyMapBasicChange(e, key)} />
        </div>
      ))}
      ...
      {Object.keys(bodyMapTailor).map((key) => (
        <div key={key}>
          <label> {key}: </label> <input type="text" value={bodyMapTailor[key]} onChange={(e) => handleBodyMapTailorChange(e, key)} />
        </div>
      ))}
